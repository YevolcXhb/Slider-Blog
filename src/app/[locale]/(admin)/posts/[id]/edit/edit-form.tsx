...
export default function EditPostForm({ post, categories, tags }: EditPostFormProps) {
  ...
  const [content, setContent] = useState(post.content_mdx);
  const [excerpt, setExcerpt] = useState(post.excerpt);
  const [categoryId, setCategoryId] = useState(post.category_id?.toString() ?? "");
  const [selectedTags, setSelectedTags] = useState<number[]>(post.tag_ids);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {tags.length > 0 ? (
              ...
              <p className="text-xs text-white/40">{t("no_tags_available")}</p>
            )}
          </div>
