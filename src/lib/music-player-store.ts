...
function ensureAudio() {
  ...
  if (saved) {
    ...
    if (
      ...
      saved.playMode === "repeat" ||
      saved.playMode === "shuffle"
    ) {
...
export function nextTrack() {
  ...
  if (state.playMode === "shuffle") {
    ...
    const next = state.currentIndex < state.playlist.length - 1 ? state.currentIndex + 1 : 0;
    loadTrack(next, true);
  }
}
...
export function prevTrack() {
  ...
  const prev = state.currentIndex > 0 ? state.currentIndex - 1 : state.playlist.length - 1;
  loadTrack(prev, true);
}
...
export function cyclePlayMode() {
  const next: PlayMode =
    state.playMode === "sequence" ? "repeat" : state.playMode === "repeat" ? "shuffle" : "sequence";
