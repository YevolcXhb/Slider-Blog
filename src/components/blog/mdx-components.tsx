...
const MermaidRenderer = lazy(() =>
  import("./mermaid-renderer").then((mod) => ({
...
function MermaidBlock({ code }: { code: string }) {
  return (
    <div className="my-6 flex justify-center">
      <Suspense
        fallback={
          <div className="h-32 w-full animate-pulse rounded-lg bg-black/5 dark:bg-white/5" />
        }
      >
...
function KatexBlock({ code, display = false }: { code: string; display?: boolean }) {
  return (
    <Suspense
      fallback={
        <span className="inline-block h-6 w-24 animate-pulse rounded bg-black/5 dark:bg-white/5" />
      }
    >
