    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      ...
      <div className="card-base p-5 md:p-6 flex flex-col">
        <div className="flex items-center gap-2 pb-3 mb-1 border-b border-(--line-divider)">
          ...
          <span className="text-base font-bold text-75 transition">
            {t("relatedPosts")}
          </span>
          ...
          relatedPosts.map((post, idx) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-75 truncate transition group-hover:text-(--primary)">
                  {post.title}
                </div>
                <div className="flex items-center gap-1.5 text-xs text-30 transition mt-0.5">
                  {post.category && (
                    <span className="shrink-0 px-1.5 py-0.5 rounded-sm bg-black/5 dark:bg-white/10 text-50">
                      {post.category.name}
                    </span>
                  )}
                  <span className="truncate">
                    {post.excerpt ||
                      formatYMD(post.published_at ?? post.created_at)}
                  </span>
                </div>
              </div>
              <ChevronRight className="shrink-0 text-xl text-30 opacity-60 transition group-hover:text-(--primary) group-hover:translate-x-0.5" />
            </Link>
          ))
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center min-h-40 text-30 transition">
