...
function SiteInfoWidget({ widgetConfig, siteInfo, className, style }: SiteInfoWidgetProps) {
  ...
  const mainItems = [
    ...
    {
      icon: Clover,
      label: labels.siteInfoBlogVersion,
      value: `Slider Blog v${siteInfo.blogVersion}`,
  ...
  const detailItems = [
    ...
    {
      icon: Rocket,
      label: labels.siteInfoFrameworkVersion,
      value: `Next.js v${siteInfo.nextVersion}`,
    },
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-col gap-2">
          {mainItems.map((item) => (
            <div
              key={item.label}
              className="flex items-center justify-between rounded-lg bg-neutral-100/60 px-3 py-2 dark:bg-neutral-800/50"
            >
              <div className="flex items-center gap-2.5">
                <div className="text-lg text-[var(--primary)]">
              ...
              {item.href ? (
                <a
                  ...
                  className="max-w-[55%] truncate text-right text-xs font-bold text-neutral-900 transition-colors hover:text-[var(--primary)] dark:text-neutral-100"
                  title={item.value}
                >
                  {item.value}
                </a>
                ...
                <span
                  className="max-w-[55%] truncate text-right text-xs font-bold text-neutral-900 dark:text-neutral-100"
                  title={item.value}
                >
                  {item.value}
                </span>
          ...
          <div className="grid grid-cols-2 gap-2">
            {detailItems.map((item) => (
              <div
                key={item.label}
                className={cn(
                  "flex flex-col items-center gap-1 rounded-lg bg-neutral-100/60 px-1 py-2 dark:bg-neutral-800/50",
              ...
              >
                ...
                <span
                  className={cn(
                    "w-full truncate text-center text-sm font-bold text-neutral-900 dark:text-neutral-100",
