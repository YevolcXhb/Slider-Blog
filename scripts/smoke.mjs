...
async function req(
  method,
  pathname,
  { headers = {}, body, redirect = "manual", timeoutMs = TIMEOUT_MS } = {},
) {
...
async function check1() {
  ...
  record(
    ...
    evidenceOf(res, res.text, [
      "json.status : " + JSON.stringify(parsed.value && parsed.value.status),
    ]),
...
async function check2() {
  ...
  if (hits429 === 0) {
    record(
      2,
      "FAIL",
      "GET /api/health/db 快速连打出现 429（F14 限流）",
      log,
      "连打 " + RL_BURST + " 次未出现任何 429；api 配额为 10 次/秒，预期应触发",
    );
    return;
  }
...
async function resolvePostId() {
  ...
  if (cal.status === 200 && Array.isArray(calJson.value)) {
    for (const p of calJson.value) {
      if (p && p.id != null)
        cands.push({ id: String(p.id), source: "GET /api/calendar/posts?locale=zh" });
    }
  }
  if (cands.length === 0)
    return { id: null, source: "未探测到（calendar 无数据）", calStatus: cal.status };
...
async function probeDbState() {
  ...
  const evidence = ["DB 探针 : GET /api/health/db → " + res.status + " " + clip(res.text, 200)];
  let cause;
...
async function check3() {
  ...
  const probeInfo = [
    "postId 来源 : " + resolved.source + (resolved.total ? "（共 " + resolved.total + " 篇）" : ""),
  ];
  ...
  if (isServerError(res)) {
    ...
    record(
      ...
      "SKIP",
      "GET /api/comments 响应 JSON 不含 author_email / 邮箱形态（F2/F10）",
      probeInfo.concat(attemptLog, evidenceOf(res, res.text), db.evidence, [
        "判定 : 服务端 5xx，环境原因 → SKIP",
      ]),
  ...
  const emailHits = valueStrings.map((s) => ({ s, m: s.match(EMAIL_RE) })).filter((x) => x.m);

  const comments = payload && Array.isArray(payload.comments) ? payload.comments : null;
  const pass =
    res.status === 200 && comments !== null && keyHits.length === 0 && emailHits.length === 0;
  ...
  const detail = [
    ...
    "comments[0] 键集合 : " +
      JSON.stringify(comments && comments[0] ? Object.keys(comments[0]) : null),
...
async function check4() {
  ...
  if (!GATEWAY_SECRET) {
    record(
      4,
      "SKIP",
      "未登录 GET /zh/dashboard 应重定向登录页（需经网关）",
      ev.concat([
        "--- 经网关访问（情形判定）---",
  ...
  if (!gated.ok) {
    record(
      4,
      "FAIL",
      "未登录 GET /zh/dashboard 应重定向登录页（经网关）",
      ev.concat(["error : " + gated.error]),
    );
    return;
  }
...
async function check5() {
  ...
  if (caseA && caseB === null) {
    record(
      5,
      status,
      "网关 fail-closed：4000 端口 /zh/dashboard 无密钥必须 404",
      ev,
  ...
  record(
    5,
    status,
    "网关 fail-closed：无密钥 404、带正确密钥不 404（F6/F11）",
    ev,
    caseA && caseB === true
...
async function check6() {
  ...
  record(
    6,
    status,
    "F14 遗留疑点：匿名 GET /api/tags 与 /api/categories 是否 200 且含真实数据（只给证据）",
    ev,
    note,
  );
}
...
async function check7() {
  ...
  for (let i = 1; i <= CALENDAR_LOOPS; i += 1) {
    ...
    ev.push(
      "#" +
        ...
        res.status +
        " x-nextjs-cache=" +
        (cache["x-nextjs-cache"] || "(absent)") +
        " age=" +
        (cache["age"] || "(absent)") +
        " cache-control=" +
        (cache["cache-control"] || "(absent)") +
        " x-vercel-cache=" +
        (cache["x-vercel-cache"] || "(absent)") +
        " items=" +
        count +
        " " +
        res.ms +
    ...
    "原始响应头（第 1 次，全部）：" +
      JSON.stringify(
        cacheHeaderFull(
          await req(
            "GET",
            "/api/calendar/posts?locale=zh" + (CACHE_BUST ? "&_bust=" + Date.now() : ""),
  ...
  if (serverErr || nonEmpty.length === 0) {
    ...
    status = "PASS";
    note =
      "只记录事实，不下结论：观测到 Next.js 缓存响应头，见上方各行。" +
    ...
    note =
      "只记录事实：连续 " +
      CALENDAR_LOOPS +
      " 次请求均未观测到 x-nextjs-cache / age / x-vercel-cache " +
      ...
      "Dynamic server usage: Route /api/calendar/posts couldn" +
      String.fromCharCode(39) +
      "t be rendered statically because it used `request.headers`。" +
  ...
  record(
    7,
    status,
    "F18 遗留疑点：GET /api/calendar/posts?locale=zh 连续请求是否命中 ISR（只给证据）",
    ev,
    note,
  );
}
...
async function check8() {
  ...
  if (serverErr) {
    status = "SKIP";
    note =
      "SKIP 原因：/api/categories 或 /api/tags 返回 5xx（环境缺数据库）。但请注意：" +
  ...
  record(
    8,
    status,
    "F17 回归点：GET /api/categories 与 /api/tags 必须可达（200），且无 locale 前缀",
    ev,
    note,
  );
}
...
async function main() {
  ...
  console.log(
    "模式         : " + mode + (mode === "gateway-only" ? "（只跑第 5 项网关检查）" : ""),
  );
  ...
  if (!pre.ok) {
    ...
    record(
      ...
      "SKIP",
      "服务可达性预检",
      ["GET " + BASE_URL + "/api/health → " + pre.error],
      "SKIP 原因：目标服务未启动/不可达。",
    );
    summarize();
    process.exitCode = 2;
